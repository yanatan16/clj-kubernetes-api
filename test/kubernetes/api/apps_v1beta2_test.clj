...
(deftest stateful-set-test
  (testing "creation of stateful-sets"
    (let [{:keys[kind metadata]} (<!! (a-v1beta2/create-namespaced-stateful-set ctx stateful-set nsopt))]
      (is (= kind "StatefulSet"))
      (is (= (:name metadata) stateful-set-name))))

  (testing "listing stateful-sets"
    (let [stateful-sets (<!! (a-v1beta2/list-namespaced-stateful-set ctx nsopt))]
  ...
  (testing "reading single stateful-set"
    (let [{:keys[kind metadata]} (<!! (a-v1beta2/read-namespaced-stateful-set ctx (assoc nsopt :name stateful-set-name)))]
      (is (= kind "StatefulSet"))
      (is (= (:name metadata) stateful-set-name))))

  (testing "deleting stateful-set"
    (let [_ (<!! (a-v1beta2/delete-namespaced-stateful-set ctx {} (assoc nsopt :name stateful-set-name)))
          {:keys [reason]} (<!! (a-v1beta2/read-namespaced-stateful-set ctx (assoc nsopt :name stateful-set-name)))]
      (is (= "NotFound" reason)))))
